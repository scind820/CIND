...
df["genre"] = [str(x).replace(" ","") for x in df["genre"]]
df["genre"] = [str(x).replace(",",", ") for x in df["genre"]]
...
df["Rating"] = pd.to_numeric(df["Rating"], downcast="float").round(1)
...
names = df["Title"].drop_duplicates()
...
app.layout = dbc.Container(
    [
        ...
        description,
        html.Hr(),
        html.H2("Movies Reccomended For You!"),
        fuzz_table(),
